...
export default function App() {
  ...
  if (!sesion && !mostrarLogin) {
    return <div className='min-h-screen bg-slate-100 p-4 md:p-8'><div className='max-w-5xl mx-auto bg-white p-6 rounded-2xl border border-slate-200 shadow-sm'>
      <div className='flex justify-between items-start gap-4'><div><h1 className='text-3xl font-bold'>FinanzaPro Simple</h1><p className='text-slate-600 mt-2'>Solicita asesoría de inversión y un comercial contactará contigo.</p></div><button onClick={() => setMostrarLogin(true)} className='bg-slate-200 px-4 py-2 rounded-lg'>Acceso agentes</button></div>
      ...
      <form onSubmit={enviarLead} className='grid md:grid-cols-2 gap-3 mt-5'>
        <input className='border p-2 rounded-lg' placeholder='Nombre completo' value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} />
        <input className='border p-2 rounded-lg' placeholder='Correo electrónico' value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
        <input className='border p-2 rounded-lg' placeholder='Teléfono' value={form.telefono} onChange={(e) => setForm({ ...form, telefono: e.target.value })} />
        <input className='border p-2 rounded-lg' type='number' min='0' placeholder='Inversión estimada (€)' value={form.inversion} onChange={(e) => setForm({ ...form, inversion: e.target.value })} />
